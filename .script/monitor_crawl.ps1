# Minute-by-minute crawl monitor report for remote GHC (run ONCE per poll)
$ErrorActionPreference = "Continue"
$ts = Get-Date -Format "HH:mm:ss"
$stateFile = "C:\Users\GHC\crawl_monitor_state.json"

$prev = @{
    channels = 0
    logLines = 0
    queryIdx = 0
    queryTotal = 0
    unique = 0
    phase = "search"
    ts = (Get-Date).ToString("o")
}
if (Test-Path $stateFile) {
    try {
        $j = Get-Content $stateFile -Raw | ConvertFrom-Json
        $prev.channels = [int]$j.channels
        $prev.logLines = [int]$j.logLines
        $prev.queryIdx = [int]$j.queryIdx
        $prev.queryTotal = [int]$j.queryTotal
        $prev.unique = [int]$j.unique
        $prev.phase = [string]$j.phase
        $prev.ts = [string]$j.ts
    } catch {}
}

$proc = Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -and $_.CommandLine -match 'crawler.py' }
$status = if ($proc) { "RUNNING" } else { "NOT_RUNNING" }
$pidVal = if ($proc) { $proc.ProcessId } else { 0 }

$logPath = "C:\Dev\XLab_YoutubeChanel\crawl_full.log"
$logText = ""
if (Test-Path $logPath) {
    $logText = Get-Content $logPath -Raw -ErrorAction SilentlyContinue
}

$queryIdx = 0
$queryTotal = 0
$unique = 0
$phase = "search"
$resolveDone = 0
$resolveTotal = 0
$unknown = 0
$lastLine = ""

if ($logText) {
    $lines = $logText -split "`r?`n"
    $lastLine = ($lines | Where-Object { $_.Trim() -ne "" } | Select-Object -Last 1)
    # [VN] [12/162] q='...' +n unique=N
    $m = [regex]::Matches($logText, "\[VN\] \[(\d+)/(\d+)\] .* unique=(\d+)")
    if ($m.Count -gt 0) {
        $last = $m[$m.Count - 1]
        $queryIdx = [int]$last.Groups[1].Value
        $queryTotal = [int]$last.Groups[2].Value
        $unique = [int]$last.Groups[3].Value
        $phase = "search"
    }
    # [VN] resolve years for X new
    if ($logText -match "resolve years for (\d+)") {
        $resolveTotal = [int]$Matches[1]
        $phase = "resolve"
    }
    # [VN] year-resolve 50/111 unknown=0
    $m2 = [regex]::Matches($logText, "year-resolve (\d+)/(\d+) unknown=(\d+)")
    if ($m2.Count -gt 0) {
        $last2 = $m2[$m2.Count - 1]
        $resolveDone = [int]$last2.Groups[1].Value
        $resolveTotal = [int]$last2.Groups[2].Value
        $unknown = [int]$last2.Groups[3].Value
        $phase = "resolve"
    }
    if ($logText -match "Saved summary") {
        $phase = "done"
    }
}

$channels = 0
$yearsFilled = 0
if (Test-Path "C:\Dev\XLab_YoutubeChanel\Vietnam") {
    Get-ChildItem "C:\Dev\XLab_YoutubeChanel\Vietnam" -Directory -ErrorAction SilentlyContinue | ForEach-Object {
        $f = Join-Path $_.FullName "channels.txt"
        if (Test-Path $f) {
            $n = @(Get-Content $f -ErrorAction SilentlyContinue | Where-Object { $_.Trim() -ne "" }).Count
            $channels += $n
            if ($n -gt 0) { $yearsFilled++ }
        }
    }
}

$elapsedMin = 1.0
try {
    $prevDt = [datetime]::Parse($prev.ts)
    $elapsedMin = [Math]::Max(0.1, ((Get-Date) - $prevDt).TotalMinutes)
} catch {}

$chDelta = $channels - [int]$prev.channels
$uqDelta = $unique - [int]$prev.unique
$qDelta = $queryIdx - [int]$prev.queryIdx
$chSpeed = [Math]::Round($chDelta / $elapsedMin, 1)
$uqSpeed = [Math]::Round($uqDelta / $elapsedMin, 1)
$qSpeed = [Math]::Round($qDelta / $elapsedMin, 2)

$pct = 0
if ($phase -eq "search" -and $queryTotal -gt 0) {
    $pct = [Math]::Round(100.0 * $queryIdx / $queryTotal, 1)
} elseif ($phase -eq "resolve" -and $resolveTotal -gt 0) {
    $pct = [Math]::Round(100.0 * $resolveDone / $resolveTotal, 1)
} elseif ($phase -eq "done") {
    $pct = 100
}

# persist state
$state = [ordered]@{
    channels = $channels
    logLines = if ($logText) { ($logText -split "`r?`n").Count } else { 0 }
    queryIdx = $queryIdx
    queryTotal = $queryTotal
    unique = $unique
    phase = $phase
    ts = (Get-Date).ToString("o")
}
$state | ConvertTo-Json | Set-Content -Path $stateFile -Encoding UTF8

Write-Output "=== BAO CAO GHC $ts ==="
Write-Output "may=remote-ghc status=$status pid=$pidVal"
Write-Output "phase=$phase progress=$pct%"
if ($phase -eq "search") {
    Write-Output "query=$queryIdx/$queryTotal unique_channels=$unique"
    Write-Output "toc_do: +$qDelta query/phut ($qSpeed q/min) | +$uqDelta kenh/phut ($uqSpeed ch/min)"
} elseif ($phase -eq "resolve") {
    Write-Output "year_resolve=$resolveDone/$resolveTotal unknown=$unknown unique=$unique"
    Write-Output "toc_do: +$chDelta kenh_luu/phut ($chSpeed ch/min)"
} else {
    Write-Output "channels_saved=$channels years_filled=$yearsFilled"
}
Write-Output "channels_saved_total=$channels years_with_data=$yearsFilled"
Write-Output "last: $lastLine"
Write-Output "=== END ==="
